...
def remove_mean_all(dataloaders, image_dataset, dataset_size, curr_path):
    train_loader = torch.utils.data.DataLoader(image_dataset, batch_size=dataset_size, num_workers=4)
    ...
    #     axes[1,i].hist(images[i,:,:,:].numpy().ravel(), bins=100, range=[-1,1])
    #     axes[1,i].yaxis.set_visible(False) # Hide only x axis
        a = stats.mode(images[i,:,:,:].numpy().ravel())
        images[i,:,:,:] = torch.from_numpy(images[i,:,:,:].numpy() - a[0])
        plt.imshow(np.squeeze(images[i,:,:,:]), cmap='gray'), plt.axis('off')
        fig.savefig(os.path.join(curr_path, "data/trans", "trans_" + str(i) + ".png"), bbox_inches='tight',  pad_inches = 0)
        fig.clf() #87912

phase = 'train'
curr_path	 = os.getcwd()
dataset_dir      = os.path.join(curr_path, "data/normalized")
testset_dir      = os.path.join(curr_path,"data/trans_testset")
batch_size       = 128
validation_split = 0.2 # -- split training set into train/val sets
n_epochs           = 200
code_size_range = [100] #dimension of the latent space
patience = 5
...
dataloaders   = {
    ...
    'test': torch.utils.data.DataLoader(test_dataset,  batch_size=dataset_size_test, num_workers=4, shuffle=False),
    }

# # Check if normalization worked
# if remove_all==1:
#     remove_mean_all(dataloaders, image_dataset. dataset_size, curr_path)
    ...
    for momentum in momentum_range:
        for lr in lr_range:
            for weight_decay in weight_decay_range:
                ...
                string_name = structure_net + '_' + str(n_epochs) + 'epochs_codeSize' + str(code_size) + '_lr' + str(lr) + '_weightDecay' + str(weight_decay) + '_split' + str(validation_split) + '_moment' + str(momentum) + '_adam_afterNorm'
                ...
                def train_model(model, batch_size, scheduler, dataloaders, patience, n_epochs):
                    ...
                    for epoch in range (1, n_epochs+1):
                        ...
                        for itr, (images, labels) in enumerate(dataloaders['train']):
                            ...
                            # print('Checking sizes: \n output_test: {}\n '.format(out.element_size() * out.nelement()))
                ...
                images, labels = dataiter.next()
                print('Dim of val_set images: {}'.format(images.shape))
                print('max {}, min {}\n'.format(torch.max(images),torch.min(images)))
                images = Variable(images).cuda()
                del dataiter

                #Get sample outputs
                output, _ = model(images)
                print('Dim of output_val image: {}'.format(output.shape))
                print('max {}, min {}\n'.format(torch.max(output),torch.min(output)))
                ...
                for images1, row in zip([images1, output1], axes):
                ...
                print('Dim of test images: {}'.format(images.shape))
                print('max {}, min {}\n'.format(torch.max(images),torch.min(images)))
                images = Variable(images).cuda()
                del dataiter
                print('Checking sizes: \n images_test: {}\n '.format(images.element_size() * images.nelement()))
                #Get sample outputs
                output, code = model(images)
                print('Dim of output image: {}'.format(output.shape))
                print('max {}, min {}\n'.format(torch.max(output),torch.min(output)))
                #Prep images for display
                images1 = images.cpu().numpy()
                del images
                #Output is resized in a batch of images
                output = output.view(dataset_size_test,1,436,436)
                #use deatch when it's an output that requires grad
                print('Checking sizes: \n output_test: {}\n '.format(output.element_size() * output.nelement()))
                output1 = output.detach().cpu().numpy()
                del output
                ...
                # output, code = model(images)
                # print('Dim of raw sample image: {}\n'.format(output.shape))
                # print('max {}, min {}'.format(torch.max(output),torch.min(output)))
                ...
                del data_phate, phate_operator, code, labels
                torch.cuda.empty_cache()
